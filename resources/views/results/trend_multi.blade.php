...
<body>
    ...
    <button id="var-in-one-graph-btn" class="btn btn-primary">Display the trend of variables side byside</button>
    <div id = "var-in-one-graph-container">
        <h4> Comparing the trend of each variable</h4>
        <div class="container">
            <div class="row">
                <div id="var-in-one-graph-chart" class="col-md-12">
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function createCombinedChart(data) {
            ...
            const seriesData = data.colname.map((col) => {
                const logData = cleanData(data.trend[col]).map(value => value !== null ? Math.log(value) : null);
                return {
                    name: col,
                    data: logData
                };
            ...
            const options = {
                ...
                title: {
                    text: 'Trend Comparison of All Variables (Logarithmic Scale)',
                    align: 'left'
                },
                tooltip: {
                    enabled: true,
        ...
        document.getElementById('var-in-one-graph-btn').addEventListener('click', function() {
            ...
            createCombinedChart(data); // Call function to create combined chart
        });
